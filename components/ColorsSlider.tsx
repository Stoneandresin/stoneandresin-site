  ...
  return (
    <section className={cx("py-10", className)}>
      <div className="container mx-auto px-4">
        {showHeading && <h2 className="text-2xl md:text-3xl font-extrabold mb-6">Choose Your Vuba Blend</h2>}
        ...
        <div className="embla relative">
          <div className="embla__viewport" ref={emblaRef}>
            <div className="embla__container -ml-3 flex">
              {colors.map((c, i) => {
                const src = getSrc(c)
                return (
                  <div className="embla__slide pl-3 basis-[80%] sm:basis-[48%] md:basis-[33%] lg:basis-[25%] shrink-0" key={`${c.name}-${i}`}>
          ...
          <div className="mt-2 flex items-center gap-2">
            <button type="button" aria-label="Previous" onClick={scrollPrev} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">
              ‹
            </button>
            <button type="button" aria-label="Next" onClick={scrollNext} className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">
              ›
            </button>
            <div className="ml-auto flex items-center gap-1.5">
              {scrollSnaps.map((_, i) => (
                <button
                  ...
                  onClick={() => scrollTo(i)}
                  className={cx("h-2.5 w-2.5 rounded-full transition", selectedIndex === i ? "bg-indigo-600" : "bg-slate-300 hover:bg-slate-400")}
                />
              ))}
            </div>
          </div>
        ...
        <p className="mt-6 text-sm copy-muted">
          See a blend you love? <a className="underline underline-offset-2" href="/#contact">Request a sample & site visit →</a>
        </p>
      </div>
    </section>
  )
